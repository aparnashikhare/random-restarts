...
/*
 * This file contains the matrix configuration for each of the Switches
 * Contains methods to build encrypt switches from respective decrypt switches
 */

public class PurpleSettingsData {
	

	final static int[][] sixesData={
		{2, 1, 3, 5, 4, 6},
		 {6, 3, 5, 2, 1, 4},
		 {1, 5, 4, 6, 2, 3},
		 {4, 3, 2, 1, 6, 5},
	...
	final static int[][] twentiesOneData={
	...
	final static int[][] twentiesTwoData={
	...
	final static int[][] twentiesThreeData=
	...
	//Three dimesional matrix consisting of four switches
	static int[][][] decryptData={sixesData,twentiesOneData,twentiesTwoData,twentiesThreeData};
	
	static int[][][] encryptData=new int[4][][];
	
	//static block that calls the method to build encrypt switch data from the decrypt switches
	static {
		constructEncryptReverseTable();
		encryptData = buildEncryptData();
	}
	//builds a decrypt data using the initial settings
	public static void constructEncryptReverseTable()
	{
		for(int[][] matrix:decryptData)
		{
	...
	// Creates encrypt switches matrix for the particular decrypt switch
	public static int[][] buildEncryptSwitchesTable(int[][] decryptSwitches)
	{
		int[][] encryptSwitches=null;
		if(decryptSwitches == sixesData)
		{
			encryptSwitches=new int[decryptSwitches.length][6];
		}
		else
		{
			encryptSwitches=new int[decryptSwitches.length][20];
		}
		
		/*System.out.println("decrypt table : ");
		for (int i = 0 ; i< decryptSwitches.length; i ++) {
			for (int j =0; j< decryptSwitches[i].length; j++ ){
				System.out.print (decryptSwitches[i][j] +",");
			}
			System.out.println("");
		}*/
		...
		for(int i=0;i<decryptSwitches.length;i++)
		{
			int[] level = decryptSwitches[i];
			int[] reciprocalLevel = new int[level.length];
			
			for (int k = 0 ; k < level.length; k ++) {
				reciprocalLevel[k] = getIndex(level, k);
			}
			
			encryptSwitches[i] = reciprocalLevel; 
		}
		
		/*System.out.println("encrypt table : ");
		for (int i = 0 ; i< encryptSwitches.length; i ++) {
			for (int j =0; j< encryptSwitches[i].length; j++ ){
				System.out.print (encryptSwitches[i][j] +",");
			}
			System.out.println("");
		}*/
		
		return encryptSwitches;
	}
	//creates a reciprocal encrypt table using the decrypt table
	private static int getIndex(int[] level, int key) {
	...
	//Builds the three dimensional encrypt data using buildEncryptSwitchesTable method
	public static int[][][] buildEncryptData()
	{
		for(int noOfSwitches=0;noOfSwitches<4;noOfSwitches++)
		{
			encryptData[noOfSwitches]=buildEncryptSwitchesTable(decryptData[noOfSwitches]);
